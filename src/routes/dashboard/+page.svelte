<script lang="ts">
    ...
    const defaultActions = [
        ...
        { id: "bookShigra", label: "Book - Shigra Darshan", site: "/dashboard/shigra", button_color: "light" },

        { id: "bookLocalide", label: "Book - Localide Darshan", site: "/dashboard/localide", button_color: "light" },
        { id: "bookBhasm", label: "Book - Bhasm Arti", site: "/dashboard/bhasmarti", button_color: "light" },
    ];
    ...
    <Card class="w-full max-w-2xl text-center p-10">
        ...
        <div class="grid gap-3 mt-4">
            {#each defaultActions as action (action.id)}
                <Button color={action.button_color} onclick={() => onActionClicked(action.site)} pill>
                    {action.label}
                </Button>
            {/each}
        </div>
